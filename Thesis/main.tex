...
%TODO Describe what we do differently (LLama, apply the pipeline, provide link to code) & mention type of replication from related work
%TODO: Citations
For our replication, we decided against using GPT-4 for predicting the results, since the usage of a closed-source model such as GPT-4 comes with several downsides, including cost, restricted access to model parameters and no guarantee of long-time availability. These downsides could potentially hinder the replicability of the results (CITE). Instead, we use the open-source LLama 3.1 with eight billion parameters, since it has shown competitive performance on related problems (CITE).

%TODO Describe next sections
