...
\textit{Reliability of Measures}: the applicability of using the \newecser pipeline is hard to measure, since our focus is on improving replicability by increasing transparency, but replicability is not a measurable quantity. There could be subjective disagreement by other researchers whether the replication has demonstrated the applicability of the pipeline. We minimized this threat by only including widely-supported recommendations into the pipeline, which means most researchers should support the carried out steps in the replication.
