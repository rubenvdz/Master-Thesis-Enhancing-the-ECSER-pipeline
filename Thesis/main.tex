...
Since LLMs are often updated after release, the exact LLM version, configuration and experiment dates should be reported to make it easier for other researchers to replicate findings \cite{baltes2025guidelinesempiricalstudiessoftware}. Additionally, when using fine-tuning, the fine-tuning data and weights should be shared if possible.

\textbf{S3. Design prompts.}\\
\infobox{Optional}{This step only applies when using prompts to guide LLM outputs, instead of using LLM embeddings with traditional classifiers.}

Prompt engineering is an important part of interacting with LLMs, since the prompt design can have a substantial effect on the performance of the LLM. We split the process of designing a prompt into five steps, similar to \textcite{Marvin2024}.

1. Define the goal of the prompt:

2. Choose the right prompt engineering approach:
