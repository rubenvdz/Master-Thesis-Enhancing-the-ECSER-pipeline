...
%To answer the question, we enhanced the existing ECSER pipeline \cite{Dellanna2022} to include the use of LLMs, taking into consideration recommendations from the fields of LLM4SE and ML4SE, as well as relevant statistical or methodological research in other fields (see section \ref{Related Work}). 

%Table \ref{table:additions} includes a summary of the additions that were made to the ECSER pipeline, with the primary supporting references for the additions in brackets.
%
%\begin{table}[H]
%    \centering
...
Starting with the evaluation metrics that are commonly used and recommended in the field \cite{Dellanna2022,hou2024}, 17 papers reported the precision, recall and F-score of the results, while only 11 papers reported the accuracy and only four papers reported the full confusion matrix. Further, some authors recommend reporting the Matthews Correlation Coefficient (MCC) and suggest the metric is more balanced than the F-score, since it takes the full confusion matrix into account \cite{Chicco2020}, but it was not reported in any of the papers.
% REMOVED: While the precision, recall and F-score are often more informative for the usefulness of a model compared to the accuracy, by not providing the accuracy or the full confusion matrix, relevant information is lost that cannot be obtained from just three metrics, which prevents further comparisons between models that do provide the full matrix.
