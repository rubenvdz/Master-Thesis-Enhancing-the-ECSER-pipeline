...
\begin{center}
\begin{abstract}
The various research papers in the field of Software Engineering (SE) that use classification algorithms, LLMs, or other machine learning methods to obtain their results differ in how many and which evaluation metrics are reported, whether or not significance tests are performed, and which steps are taken to aid reproducibility. The ECSER pipeline was designed to mitigate this issue by providing a step-by-step pipeline that researchers can use to report their results when classification algorithms are used, and the pipeline was empirically shown to be effective in replicating the findings of several studies, as well as producing additional findings and occasionally contradicting the findings of the original papers. However, the ECSER pipeline was designed for evaluating simple classifiers and gives no specific recommendations for LLM classifiers, despite LLMs being an increasingly popular choice for classification tasks. 
This thesis expands the ECSER pipeline for the use of LLMs by adding recommendations from LLM4SE research and related fields. First, an exploratory mapping study was conducted of SE studies released in 2024 that use LLM classifiers, summarising which steps were taken and which metrics were (or weren't) reported, showing lacklustre reporting standards and a pattern of unsubstantiated conclusions. To address these issues, we designed the \newecser pipeline, an enhanced version of the ECSER pipeline for the use of LLMs, which includes recommendations for prompt design and comparison, and the evaluation of the calibration, fairness, robustness and sustainability of LLMs. Lastly, in order to evaluate the applicability of the new pipeline, we conducted a replication study on the prediction of Python tests without execution using the pipeline, showing that using the pipeline improved the replicability and comprehensiveness of the results compared to the original paper, while unable to corroborate some of the original conclusions.
\end{abstract}
\vfill 
...
Originally, software engineering (SE) research was primarily focused on theoretical aspects such as the development of algorithms and the practical development of software \cite{felderer2020evolutionempirical}. However, the way that SE research is being conducted has shifted with the introduction of empirical software engineering, involving the use of experimental methods. Since its infancy, many researchers have reported that there is a poor standard of empirical software engineering, which inhibits the usefulness of results and impairs reproducibility \cite{kitchenham2002,Menzies2012}. To treat the observed poor standard of empirical SE, some researchers have called for a more systematic approach to conducting and evaluating experiments. The ECSER (Evaluating Classifiers in Software Engineering Research) pipeline, introduced by \textcite{Dellanna2022}, provides a systemic pipeline for training, validating and testing ML classifiers and analysing their results. ECSER is based on recommendations from the field of machine learning for software engineering (ML4SE) and is designed to be easy to use for SE researchers, regardless of expertise with machine learning methods. The ECSER pipeline has seen some empirical validation in the form of two replication studies, in requirements engineering and software testing, where \citeauthor{Dellanna2022} were able to confirm and strengthen some findings, as well as discover additional findings or findings that contradict the original ones. Figure \ref{fig:ECSER} shows the steps of the ECSER pipeline.
...
% Introduce LLMs
In recent years, large language models (LLMs) have become an increasingly relevant tool in software engineering, allowing researchers to accomplish complex tasks usually reserved for humans or other machine learning (ML) methods, such as code generation and summarisation, while also showing promise for classification tasks \cite{Guo2024health,fields2024}. However, few guidelines exist for conducting and reporting on LLM research, leaving researchers on their own to decide what choices to make when designing prompts, what evaluation metrics to report in their results and the level of statistical validation needed for their comparisons. Because of this lack of guidelines, researchers using LLMs often omit important details, such as the $F_1$-score or the significance of their results \cite{guo2023survey}.

% What is the ECSER pipeline missing (LLMs) / Problems
While many of the recommendations included in ECSER also apply to LLM classifiers, it is ultimately tailored to traditional ML classifiers (such as logistic regression, SVMs and decision trees), and includes no specific recommendations for LLM classifiers. Thus, the ECSER pipeline is not sufficient for LLM research, since using LLMs for classification comes with unique considerations that are not present for traditional classifiers. For one, it is common to use pre-trained, generalised LLMs and give task-specific instructions to get the desired classification on the input data. This means that designing these instructions, called prompts, becomes a crucial part to conducting LLM research \cite{Marvin2024}. There are also unique considerations when it comes to evaluating the calibration, fairness, robustness and sustainability of the results: does the model know its own limits, does the model behave in accordance with human values and are the results resilient to adversarial prompts, such as those containing typos \cite{Woodworth2017,zhu2024}?

% Contributions that were made
This thesis addresses the lack of LLM-specific guidelines in the ECSER pipeline, by expanding it to include specific recommendations for conducting LLM classifier research. In doing so, we made the following contributions:
\begin{itemize}
    \item We conducted a mapping study to assess the current state of reporting in LLM4SE classification research, showing a pattern of incomplete reporting practices and unsubstantiated conclusions.
    \item We developed the \newecser pipeline, an expanded version of the ECSER pipeline that includes specific recommendations for conducting LLM classifier research, such as prompt engineering and the evaluation of model calibration, fairness, robustness and sustainability.
    \item We conducted a replication study on the prediction of Python test cases without execution using the \newecser pipeline, showing how the pipeline enhances the replicability and comprehensiveness of the original results, while challenging some of the original conclusions.
\end{itemize}
...
The remainder of this thesis is structured as follows. In Section \ref{Background}, we provide background information about LLMs. Section \ref{Research approach} provides and motivates the research questions. Section \ref{Related Work} discusses related work. Section \ref{RQ1} discusses the approach to answering RQ1 and the resulting mapping study. Section \ref{RQ2} discusses the approach to answering RQ2 and introduces the \newecser pipeline. Section \ref{RQ3} discusses our evaluation of the \newecser pipeline by way of a replication study, providing an answer to RQ3. Section \ref{threats-to-validity} discusses several threats to the validity of our research. Lastly, Section \ref{conclusion} summarises the findings of the research questions and discusses its limitations and directions for future work.
...
\begin{itemize}
    \item \textbf{Encoder-decoder LLMs}, such as BART \cite{lewis2019bart} and T5X \cite{roberts2022t5x}, are good at both understanding and generating language \cite{hou2024}. As such they are useful for tasks like summarisation and translation \cite{cho2014encoderdecoder, Asadi2020encoderdecoder}.
...
\subsection{Guidelines for Replication Studies in SE}
...
\textcite{chang2023} conducted another survey of the evaluation of LLMs which focuses on downstream applications, resulting in a classification of four aspects of LLM evaluation: the accuracy, containing measures of correctness such as the F1 score; the calibration, which contains measures for the degree of alignment between the predicted probabilities and actual probabilities, such as the expected calibration error (ECE); the fairness, with measures pertaining to the equal treatment of different groups, such as the equalised odds difference (EOD); and robustness, with measures pertaining to the performance of a model in the face of challenging inputs and noise, such as performance drop rate (PDR). Further, they mention several ways in which human evaluation can be used to evaluate LLMs, such as the degree to which the model outputs align with human values.
...
\textbf{S5. Test the models.}\\
After developing multiple prompts and comparing the results on the validation set, we determined that the Persona prompt has the best performance in our specific case. Since the performance can vary wildly based on specific word choice and intricacies of the data set \cite{sclar2024quantifyinglanguagemodelssensitivity,mizrahi2024stateartmultipromptllm}, this should not be read as an endorsement of the Persona prompt pattern, and we encourage researchers to conduct their own comparisons. Subsequently, we use this prompt with the test set to obtain the final results for S6-S11. 
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		\centering
		\vskip 0pt
		\infobox{Charswap attack prompt}{Act as a Python expert and consider the following \textcolor{red}{etst} of the Python Standard
			Library, version 3.10:\\
			\textlangle test\_case\textrangle\\
			\textcolor{red}{YOour} job is to figure out whether this test will \textcolor{red}{PASB} or
			FAIL. Provide the rationale before \textcolor{red}{givin} an answer. 
			Provide outputs that a \textcolor{red}{PythLon} expert would create.\\

			Answer in this exact format:\\
			Explanation: \textlangle free text\textrangle\\
			Label: \textlangle PASS $\vert$ FAIL\textrangle}
		\caption[b]{Character-level prompt attack using the charswap augmenter.}
		\label{fig:charswap}
	\end{subfigure}
	\begin{subfigure}[t]{0.48\textwidth}
		\centering
		\vskip 0pt
		\infobox{Embedding attack prompt}{Act as a Python \textcolor{red}{specialist} and \textcolor{red}{contemplate} the following test of the Python Standard
			Library, version 3.10:\\
			\textlangle test\_case\textrangle\\
			Your job is to figure out whether this test will PASS or
			FAIL. \textcolor{red}{Offer} the rationale before giving a \textcolor{red}{response}. 
			\textcolor{red}{Deliver} outputs that a Python expert would create.\\
			
			\textcolor{red}{Respond} in this exact format:\\
			Explanation: \textlangle free text\textrangle\\
			Label: \textlangle PASS $\vert$ FAIL\textrangle}
		\caption[b]{Word-level prompt attack using the embedding augmenter}
		\label{fig:embedding}
	\end{subfigure}
	\caption{The generated adversarial prompts using the original Persona prompt (Figure \ref{fig:persona_prompt}). Changes made to the original prompt are highlighted in red.}
	\label{fig:prompt_attacks}
\end{figure}
...
\textbf{Sustainability}\\
Using an LLM inherently comes with a significant energy footprint. However, several of the choices we made in our study have reduced the potential energy expenditure. In particular, our using a pre-trained model avoids the large footprint that results from the training phase of LLMs \cite{wu2022sustainableai}. Further, we have used a relatively small model, run locally, since it was not necessary to use a larger model given our focus on replicability over raw performance. Lastly, in order to keep inferences low, we used a small subset of the data for the initial testing of our prompts and used a method of confidence elicitation that does not require repeated inference. As a result of using a smaller model, we have likely used less energy than the original study, since Llama-3.1-8b has been estimated to use 0.017 Wh per query \cite{EnergyScore2026}, whereas GPT-4 has been estimated to use between 0.3 and 3 Wh per query \cite{deVries2023,epoch2025energygpt}.\\

\textbf{S9. Analyse overfitting \& degradation.}\\
Since we have not performed training or fine-tuning, we do not have information on the performance of the model on this task during the training phase, which means that we cannot analyse the level of overfitting. However, since our prompts were selected and improved using the performance on the validation set, there is a possibility that we have incidentally tailored our prompts to the intricacies of the validation set as opposed to features of the general problem. To investigate this potential issue, we calculate the degradation ($M_{test} - M_{validation}$) of the performance between the validation and test sets, shown in Table \ref{table:degradation}.

The results show a positive degradation on all metrics, meaning that the performance has actually improved in the test set compared to the validation set. However, the differences are very small compared to the variations in performance we have seen on the validation set for different prompts and the performance on different test sets (Table \ref{table:prompt_metrics} and \ref{table:test_metrics}). Therefore, we argue that the performance is quite stable between the validation and test set, suggesting that the steps of prompt design and prompt comparison did not inadvertently bias our prompt on the validation set. This result is unsurprising, since our final prompt does not incorporate any information unique to the validation set. 

\begin{table}[H]
	\centering
	\caption{Performance degradation from validation to test set.}
	\label{table:degradation}
	\begin{tabularx}{\textwidth}{|X|c|c|c|c|c|c|c|}
		\hline
		 & \textbf{Precision} & \textbf{Recall} & \textbf{Specificity} & \textbf{Accuracy} & \textbf{F1} & \textbf{F2} & \textbf{MCC}\\
		\hline
		Validation & 0.514 & 0.740 & 0.300 & 0.520 & 0.607 & 0.680 & 0.045 \\
		Test & 0.533 & 0.800 & 0.300 & 0.550 & 0.640 & 0.727 & 0.115 \\
		\hline
		\textbf{Degradation} & 0.019 & 0.060 & 0.000 & 0.030 & 0.033 & 0.047 & 0.071 \\
		\hline
	\end{tabularx}
\end{table}
...
\begin{figure}[H]
	...
	\caption{The ROC curve (blue) of the predictions on the test set. The diagonal line (orange) represents random guessing.}
	\label{fig:ROC}
\end{figure}

The ROC curve gives a visual representation of how well the model is able to differentiate the positive and negative classes across different thresholds. Figure \ref{fig:ROC} shows the ROC curve of the results of our test set. To calculate which predictions were above or below the thresholds, we used the estimated confidence values. However, we have seen that the model is poorly calibrated (S8), which means that these confidences are not very accurate. That said, we can still see that the ROC curve is consistently to the top-left of the dashed diagonal line that represents random guessing, which suggest that the model is able to differentiate the positive and negative classes on each threshold. However, visually it is clear that the model does not provide a good fit on the test data. If we calculate the area under the curve (AUC), we get a value of 0.573. If we recall that any positive fit is between 0.5 and 1, the AUC is clearly better than random chance, but still on the lower end. 

Unfortunately, the original study did not provide an ROC curve or the AUC metric, nor did they provide the necessary information to compute this information (i.e., the confidence values). Therefore, we cannot compare the fit of our model to theirs. It is likely that, given their higher accuracy and $F_1$-score, the ROC curve dominates our curve, but we cannot be certain because of the missing information.\\

\textbf{S11. Apply statistical significance tests.}\\
To be able to draw conclusions from the results we have observed so far, we must first reject the hypothesis that the results are due to random variations. The central research question of the original paper is "What is the performance of GPT-4 to predict test results?". They further divide this question into three research questions: (RQ1) Prediction for all test cases, (RQ2) Predictions by test case complexity and (RQ3) Prediction by test suite. In our case, we must rephrase the main research question as "What is the performance of LLama 3 to predict test results?". Otherwise, the three sub-questions are the same. We investigate these questions in order, applying statistical tests where necessary to substantiate our conclusions. Since we have already discussed the performance metrics in S7, we focus on the statistical testing itself. 

\textbf{RQ1: Prediction for all test cases.}\\
In total, we included 100 test cases in our test set. The results on this test set show that our model is able to predict whether the test cases will fail or pass with an accuracy of 0.550, although the $F_1$-score (0.640) and $F_2$-score (0.727) are much higher. Random guessing between two binary options would, on average, provide an accuracy of 0.5. To determine whether our model is actually able to predict the outcome of tests without execution, we must reject the null hypothesis that the predictions were made due to random guessing. 

We use an exact binomial test to assess the significance null hypothesis \cite{lancaster1961significance}, resulting in a p-value of $5.64 * 10^{-7}$. Since this p-value is lower than any reasonable significance threshold, we reject the null hypothesis and conclude that the model is able to predict test results more often than not. Whether the performance is good enough to be used in any context is out of scope of this replication. The original study did not perform statistical testing, which means that even though they almost certainly had a statistically significant results (since they had better performance metrics), our significance testing has corroborated the findings for RQ1 of the original study.

\textbf{RQ2: Prediction by test case complexity.}\\
The test set consisted of 50 simple test cases and 50 complex test cases. Note that the original study does not specify how they determined that a test was simple or complex, which makes this research question difficult to objectively analyse. However, assuming the labels are correct, we can determine that there is a difference between the performance on the simple and complex cases if we reject the null hypothesis that the results are drawn from the same distribution. 

We use the Wilcoxon Signed-Rank test to assess the significance of the null hypothesis, since it is a non-parametric test that is suitable for comparing two samples \cite{Dellanna2022,Rainio2024}, resulting in a p-value of 0.513. Given this p-value is higher than the common threshold of 0.05 (or any reasonable threshold), we do not reject the null hypothesis. This does not mean that we accept the null hypothesis that there is no difference in the performance, only that we do not have enough evidence to claim that there is. Since the original study did not perform statistical testing, we do not know whether their results for RQ2 are statistically significant and cannot corroborate their conclusions ourselves, which means that as it stands, their conclusion that it is easier for GPT-4 to predict the results of simple tests is unsubstantiated. 

\textbf{RQ3: Prediction by test suite.}\\
The test cases that make up the test suites were taken from five different Python libraries: ast, calender, csv, gzip and string. Each suite is represented by 20 samples in the test set. To determine whether the model performs better on some suites than others, we again have to reject the null hypothesis that the predictions are drawn from the same distribution. 

Since we are comparing five sets of results, and prefer a non-parametric test, we use the Friedman's test to determine the significance of the null hypothesis \cite{Dellanna2022,Rainio2024}, resulting in a p-value of 0.334. Since the p-value is higher than 0.05, we do not reject the null hypothesis. Again, we are not able to corroborate the conclusion of the original study that there is a difference in the performance between the different suites, nor have they provided statistical evidence for these claims. Therefore, these findings remain unsubstantiated.\\

For RQ2 and RQ3, it should be noted that the number of samples is quite small, which could infer a low statistical power to find statistical significance, even if there is an actual difference between the groups \cite{Cohen1992}. The original study has double the samples due to the fact that there is no split between a validation and test set, but this does not necessarily mean that the study has enough samples to discover a statistically significant effect, if it does exist. Either way, no statistical tests were conducted in the original study, so it remains unknown whether there is a significant difference between the groups in the original study, leaving their conclusions unsubstantiated.

\subsubsection{Summary of Findings}
The purpose of the replication study was to answer RQ3: how applicable is the proposed pipeline in assisting LLM4SE classification research? In this section, we summarise how applying the \newecser pipeline has assisted the prediction of test cases without execution.
\begin{enumerate}
	\item Splitting the data set into validation and test sets allowed us to analyse the generalisability of the results.
	\item The performance of the model varies a lot depending on the chosen prompt approach and prompt pattern, as well as the specific choice of words or the presence of typos, supporting the development and comparison of multiple prompts.
	\item Providing the $F_1$-score and $F_2$-score gave additional insight into the balance between the precision and recall of the results, whereas the MCC provided additional insight into the balance of all parts of the confusion matrix.
	\item Analysing the calibration shows that the model is poorly calibrated, meaning that we should not blindly trust the provided explanations.
	\item Using a smaller, open-source model allowed us to keep the carbon footprint low and made it easier to estimate the confidence of the model.
	\item The ROC curve gives a visual indication of the level of fit, showing that the model does not provide a great fit on the test data.
	\item Applying statistical testing to the original research questions supports the original conclusion that the model is able to predict the outcome of test cases without execution, but was not able to find a significant difference between simple and complex test cases or between the five test suites. 
\end{enumerate}

In conclusion, we have shown how the \newecser pipeline was able to assist the reporting of a LLM4SE classification study by producing findings that were absent in the original study, increasing the replicability of the results and providing a more critical perspective on some of the original conclusions. Many of the shortcomings that we have addressed in this replication study are also present in many other studies in the field of LLM4SE, as shown in RQ1, suggesting that the \newecser pipeline could provide similar improvements to other papers in the field. Further, we specifically chose the replicated paper for its use of prompts, since the prompt-related steps are one of the major additions compared to the original ECSER pipeline. However, some steps of the pipeline did not apply to the current replication, including the steps of fine-tuning, hyper-parameter tuning and the evaluation of the fairness (although some of these recommendations were evaluated in the replication studies of the ECSER pipeline \cite{Dellanna2022}). Therefore, a single replication study cannot provide conclusive evidence of the wider applicability of the pipeline, although it certainly provides a necessary first step. 

\section{Threats to Validity}
\label{threats-to-validity}
In this section, we identify the threats to the validity of our research using the list of possible threats by \textcite{Wohlin2024validity}, which is based on \textcite{Cook1979validity}. 

\textbf{Conclusion Validity} \\
Threats to the conclusion validity refer to the relation between the treatment (the \newecser pipeline) and the outcome (RQ3). We identify the following threats:

\textit{Reliability of Measures}: the applicability of using the \newecser pipeline is hard to measure, since our focus is on improving replicability by increasing transparency, but replicability is not a measurable quantity. There could be subjective disagreement by other researchers whether the replication has demonstrated the applicability of the pipeline. We minimised this threat by only including widely-supported recommendations into the pipeline, which means most researchers should support the carried out steps in the replication.

\textit{Reliability of Treatment Implementation}: it is possible that other researchers would have applied the \newecser pipeline differently to the replication study, since the pipeline leaves room for interpretation (e.g., the prompt approach). However, we believe our conclusions are general enough to apply to all different choices in the execution of the pipeline.

\textit{Random Irrelevancies in Experimental Setting}: using an LLM comes with the problem of non-determinism, which could risk changing the results. We have mitigated this issue by setting the temperature to 0.

\textbf{Internal Validity}\\
Threats to internal validity are unknown influences that affect the dependent variable. In our case, the following threat could have influenced the observed results in the replication study:

\textit{Selection}: we used the same data set as the original study in our replication. It is possible that the selected Python tests from the Python Standard Library are not representative of all Python tests, which could mean the results differ for Python test cases from other sources. Future research is needed to see if LLMs are able to predict the outcome of other Python test cases, or those written in other languages.

\textbf{Construct Validity}\\
Threats to the construct validity concern the generalisation of the results to the concept or theory behind the experiment. In other words, can the results in RQ3 be used to draw general conclusion about the \newecser pipeline. We identify the following threats:

\textit{Inadequate Pre-operational Explication of Constructs}: the theory included in the pipeline may be correct, but if the concepts are not clearly defined, the resulting application of the pipeline becomes muddled. We have mitigated this issue by accompanying many of our recommendations with references to more in-depth sources in case additional explanation is needed.

\textit{Mono-Operation Bias}: we conducted a single replication study, which does not apply all the steps of the pipeline. There is a risk that using a single replication study does not provide a full picture of the applicability of the pipeline. To mitigate this issue, we picked the replication study specifically to demonstrate the application of the new LLM-specific steps of prompt design and prompt comparison, since the other steps were already evaluated as part of the ECSER pipeline \cite{Dellanna2022}. 

\textit{Experimenter Expectancy}: knowing the conclusions of the original study could have inadvertently biased the results of the replication study. However, the conclusions obtained by following the \newecser pipeline actually differed from the original, since statistical significance testing was conducted that did not find significant differences for some experiments. 

\textbf{External Validity}\\
Threats to external validity limit the generalisability of the results of the experiment to the outside world. We note the following threat:

\textit{Interaction of Selection and Treatment}: the study that was selected for our replication must be representative of the field of LLM4SE for our observations to be generalisable to the field as a whole. We mitigated this threat by picking a recent study (2024) from the field of LLM4SE that was published in a reputable journal (FSE). Further, the study did not follow all the steps of the pipeline, which is the general case in the field (see RQ1).

\section{Conclusion}
\label{conclusion}
In this thesis, we enhanced the ECSER (Evaluating Classifiers in Software Engineering Research) pipeline with recommendations for the use of LLMs, resulting in the \newecser pipeline. In order to develop a pipeline that takes into account the existing state of the field of LLM4SE, we followed \textcite{Wieringa2014}'s design cycle of \textit{problem investigation} (RQ1), \textit{treatment design} (RQ2) and \textit{treatment validation} (RQ3). In the following, we briefly summarise the approach and the conclusions from each research question:

\textbf{RQ1: What is the current state of reporting in LLM4SE classification research?}
To answer RQ1 and investigate the state of reporting in the field LLM4SE, we conducted a mapping study of all the LLM4SE papers published in 2024 in three of the top SE conferences (ICSE, FSE \& TSE), extracting information about the reporting practices of these papers. The results of the mapping study showed a pattern of incomplete reporting and unsubstantiated claims, supporting the need for more comprehensive reporting guidelines that could be easily followed by researchers to increase the transparency and the replicability of the results. 

\textbf{RQ2: What resource can be developed to assist researchers in conducting and reporting on LLM4SE classification research?}
To answer RQ2 and provide a resource for LLM4SE researchers to assist the conducting and reporting of classification research, we developed the \newecser pipeline, which enhances the recommendations of the original ECSER pipeline with recommendations from the fields of LLM4SE, ML4SE and Design Science, with the intent of helping researchers avoid the problems identified in RQ1. Some steps of the pipeline were modified from the original ECSER to tailor the recommendation more towards the use of LLMs, while some new steps were developed that are specific to the use of LLMs, such as the steps of prompt design, prompt comparison and the evaluation of the calibration, fairness, robustness \& sustainability. 

\textbf{RQ3: How applicable is the proposed pipeline in assisting LLM4SE classification research?}
To answer RQ3 and evaluate the applicability of the \newecser pipeline, we conducted a replication study on the prediction of Python test results without execution \cite{10.1145/3663529.3663794}, using the proposed pipeline. This study was chosen since it was recent (2024), published in a reputable journal (FSE) and represented current practices in the field of LLM4SE. The results of the replication study show that applying the \newecser pipeline improved the generalisability of the results by analysing the degradation and ROC plot, improved the replicability by providing a more systematic approach to prompt development and comparison; and provided additional insights into the results by analysing the calibration, robustness, sustainability and reporting additional performance metrics ($F_1$, $F_2$, MCC). Further, conducting statistical testing showed that we were only able to corroborate the finding that LLMs perform better than chance on the task of predicting test results, but did not find a significant result for the difference between simple and complex test cases and the difference between the test suites, leaving the original conclusions unsubstantiated. These findings, although specific to the replicated study, show that the \newecser pipeline is able to address some of the shortcomings that were discovered in RQ1, since the shortcomings of this paper are present in many of the analysed papers. However, some of the recommendations of the pipeline did not apply to the replicated paper, such as those related to fine-tuning, hyper-parameter tuning and evaluating the fairness of the results. These steps of the pipeline require further replication studies to fully assess the wider applicability of the pipeline.

\subsection{Limitations and Future Work}
The goal of the \newecser pipeline is to be a tool for LLM4SE researchers that makes it easier to follow best-practices in the field. However, so far it has only been applied by us. It is possible that there are parts of the pipeline that are difficult to apply for some researchers, especially given the fact that LLMs are still a relatively new tool that have only recently become good enough for many tasks in SE. Further, we have only conducted a single replication study using the pipeline, focused on a single SE task and using a single LLM. Future work should therefore focus on applying the \newecser pipeline on a larger variety of tasks and LLMs, and ideally be conducted by researchers with various levels of experience with the use of LLMs, to give a more complete picture of the usefulness of the pipeline for LLM4SE researchers, which could possibly inform changes to the pipeline to make steps easier to follow or increase the comprehensiveness. 

Another limitation of the pipeline is that, as of the writing of this thesis, there are very few existing guidelines for the use of LLMs in SE, especially for classification tasks. Evidently, researchers are still figuring out the best way to use and report on LLMs for various SE tasks. Due to the high speed and volume of LLM research, it is possible that some of the recommendations of the pipeline will become outdated or incomplete as more consensus is being reached for the best way to conduct research using LLMs, or better methods are discovered. This is especially likely for the prompt engineering recommendations, since new ways to design prompts are constantly being developed, including approaches that automate the process \cite{zadenoori2025}. 

In terms of scope, the \newecser pipeline deals exclusively with the use of LLMs for classification, similar to the scope of the ECSER pipeline \cite{Dellanna2022}. However, LLMs are most commonly used for generation, and are sometimes also used for recommendation \cite{hou2024}. Despite this, few guidelines exist for the use of LLMs for generation and recommendation in the field of LLM4SE, and we could not find any resource that provides a pipeline for evaluating LLM4SE generation and recommendation research. Future research should expand the \newecser pipeline or provide a similar pipeline to include a broader range of applications that address any shortcomings of existing generation/recommendation research. 

In conclusion, the \newecser pipeline should be seen as a first attempt at collecting recommendations for conducting and reporting on LLM4SE classification research, but it is by no means a definitive set of guidelines and does not replace the sources that were used to create it, or those that will be written in the future.
